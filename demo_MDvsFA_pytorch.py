...
# 定义dataset // definir conjunto de datos
class G1G2Dataset(torch_data.Dataset):
    def __init__(self, mode):
        self.mode = mode

        if self.mode == 'train':
            self.imageset_dir = os.path.join('./data/training_data/')
            self.imageset_gt_dir = os.path.join('./data/training_data/')
        elif self.mode == 'test':
            self.imageset_dir = os.path.join('./data/test_single_frame/')
            self.imageset_gt_dir = os.path.join('./data/test_single_frame_gt/')
        else:
    ...
    def __getitem__(self, idx):
        if self.mode == 'train':
            ...
            # -1表示同时加载了alpha通道 // -1 indica que el canal alfa también esta cargado
...
class Generator1_CAN8(nn.Module):
    def __init__(self):
        ...
        # Fórmula de cálculo convolución atroz: ---, los corchetes indican floor()
    ...
    def forward(self, input_images):  # 输入[B, 1, 128, 128],输出[B, 1, 128, 128] //entrada salida
...
if __name__ == '__main__':
    ...
    # 定义3个Model // definir los 3 modelos
    g1 = Generator1_CAN8()
#    g1.cuda()
    ...
    # 定义3个优化器 // definir 3 optimizadores
    ...
    # 定义loss // definir pérdida
    loss1 = nn.BCEWithLogitsLoss()

    it = 0

    for epoch in range(0, max_epoch_num):
        # 调整学习率 // ajustar tasa de aprendizaje
        if (epoch + 1) % 10 == 0:
            ...
            for r in optimizer_g2.param_groups:
                r['lr'] *= 0.2
        # 训练一个周期 // ciclo de entrenamiento
        logger.info('Now we are training epoch {}!'.format(epoch + 1))
        total_it_per_epoch = len(trainset)
        for bt_idx, data in enumerate(trainset):
            # 训练一个batch // entrenar batch
            torch.cuda.empty_cache()  # 释放之前占用的缓存 // liberar cache
            it = it + 1
            logger.info(
                'current iteration {}/{}, epoch {}/{}, total iteration: {}, g1 lr: {}, g2 lr: {}, Dis lr: {}'.format(
                    bt_idx + 1, total_it_per_epoch, epoch + 1, max_epoch_num, it,
                    float(optimizer_g1.param_groups[0]['lr']),
                    float(optimizer_g2.param_groups[0]['lr']), float(optimizer_d.param_groups[0]['lr'])))

            # 先训练判别器 // primero entrena discriminador
            dis.train()
            g1.eval()
            g2.eval()
            ...
            # 将输入输出放到cuda上 // entrada y salida en cuda
            input_images, output_images = data['input_images'], data['output_images']  # [B, 1, 128, 128]
            #input_images = input_images.cuda(non_blocking=True).float() comentario mio
            #output_images = output_images.cuda(non_blocking=True).float() mio
            ...
            const1 = torch.ones(mini_batch_size, 1)#.cuda(non_blocking=True).float() mio
            const0 = torch.zeros(mini_batch_size, 1)#.cuda(non_blocking=True).float() mio
            ...
            disc_loss.backward()  # 将误差反向传播 // propagacion inversa del error
            optimizer_d.step()  # 更新参数 // actualizar parámemtros
            ...
            gen_loss1.backward()  # 将误差反向传播 // propagación inversa del error
            optimizer_g1.step()  # 更新参数 // actualizar parámetros

            # 再训练g2 // reentrenear g2
            dis.eval()
            g1.eval()
            g2.train()
            optimizer_g1.zero_grad()
            ...
            gen_loss2.backward()  # 将误差反向传播 // propagación inversa del error
            optimizer_g2.step()  # 更新参数 // actualizar parámetros

            if (bt_idx + 1) % 10 == 0:
                # 在测试集上测试 // conjunto de prueba
                sum_val_loss_g1 = 0
                ...
                for bt_idx_test, data in enumerate(testset):
                    ...
                    g3_out = (g1_out + g2_out) / 2  # 取均值的方式进行融合 // fusion con la media
